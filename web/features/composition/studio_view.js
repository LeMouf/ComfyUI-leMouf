...
function placementBelongsToResource(resourceId, placementKey) {
  const rid = String(resourceId || "").trim();
  const key = String(placementKey || "").trim();
  if (!rid || !key) return false;
  return key === rid || key.startsWith(`${rid}${PLACEMENT_SEGMENT_PREFIX}`);
}

function parsePlacementSegmentIndex(resourceId, placementKey) {
  const rid = String(resourceId || "").trim();
  const key = String(placementKey || "").trim();
  if (!placementBelongsToResource(rid, key)) return null;
  if (key === rid) return 0;
  const suffix = key.slice((`${rid}${PLACEMENT_SEGMENT_PREFIX}`).length);
...
function buildPlacementKey(resourceId, segmentIndex) {
  ...
  if (!rid) return "";
  if (!Number.isFinite(seg) || seg <= 0) return rid;
  return `${rid}${PLACEMENT_SEGMENT_PREFIX}${Math.round(seg)}`;
}

function nextPlacementSegmentIndex(placementMap, resourceId) {
  let maxSeg = 0;
  for (const key of placementMap.keys()) {
    const seg = parsePlacementSegmentIndex(resourceId, key);
...
function collectPlacementsForResource(placementMap, resourceId) {
  ...
  for (const [key, value] of placementMap.entries()) {
    if (!placementBelongsToResource(rid, key)) continue;
    const row = value && typeof value === "object" ? value : {};
    rows.push({
      ...row,
      placementKey: String(key),
      resourceId: rid,
      timeSec: Math.max(0, toFiniteNumber(row.timeSec) || 0),
      durationSec: Math.max(0.1, toFiniteNumber(row.durationSec) || 0.1),
      startOffsetSec: Math.max(0, toFiniteNumber(row.startOffsetSec) || 0),
    });
  }
  rows.sort((a, b) => {
    const delta = Number(a.timeSec || 0) - Number(b.timeSec || 0);
    if (delta !== 0) return delta;
    return String(a.placementKey || "").localeCompare(String(b.placementKey || ""));
  });
  return rows;
}
...
function setPlacementsForResource(placementMap, resourceId, placements) {
  const rid = String(resourceId || "").trim();
  if (!rid) return [];
  for (const key of Array.from(placementMap.keys())) {
    if (placementBelongsToResource(rid, key)) placementMap.delete(key);
  }
  const rows = Array.isArray(placements) ? placements : [];
  const out = [];
  for (let i = 0; i < rows.length; i += 1) {
    const row = rows[i] && typeof rows[i] === "object" ? rows[i] : {};
    const key = String(row.placementKey || "").trim() || buildPlacementKey(rid, i === 0 ? 0 : i);
    const normalized = {
    ...
    placementMap.set(key, normalized);
    out.push({ ...normalized, placementKey: key, resourceId: rid });
  }
  return out;
}
...
function upsertPlacementForResource(placementMap, resourceId, placementKey, patch) {
  ...
  if (!rid) return null;
  const key = String(placementKey || "").trim();
  const base = key && placementBelongsToResource(rid, key)
    ? (placementMap.get(key) || {})
    : (getPrimaryPlacementForResource(placementMap, rid) || {});
  const nextKey = key && placementBelongsToResource(rid, key)
    ? key
    : (String(base?.placementKey || "").trim() || buildPlacementKey(rid, 0));
  const merged = {
    ...base,
    ...(patch && typeof patch === "object" ? patch : {}),
  };
  const normalized = {
    trackName: String(merged.trackName || "").trim(),
    timeSec: Math.max(0, toFiniteNumber(merged.timeSec) || 0),
    durationSec: Math.max(0.1, toFiniteNumber(merged.durationSec) || 0.1),
    startOffsetSec: Math.max(0, toFiniteNumber(merged.startOffsetSec) || 0),
    placementKey: nextKey,
  };
  placementMap.set(nextKey, {
...
function splitPlacementForResource(placementMap, resourceId, placementKey, cutTimeSec, minDurationSec = 0.1) {
  const rid = String(resourceId || "").trim();
  const key = String(placementKey || "").trim();
  if (!rid || !key || !placementBelongsToResource(rid, key)) return null;
  const currentRaw = placementMap.get(key);
  if (!currentRaw || typeof currentRaw !== "object") return null;
  const current = {
    trackName: String(currentRaw.trackName || "").trim(),
    timeSec: Math.max(0, toFiniteNumber(currentRaw.timeSec) || 0),
    durationSec: Math.max(minDurationSec, toFiniteNumber(currentRaw.durationSec) || minDurationSec),
    startOffsetSec: Math.max(0, toFiniteNumber(currentRaw.startOffsetSec) || 0),
  };
...
function buildStudioData(resources, scopeKey) {
  ...
  for (const resource of resources) {
    ...
    if (!placements.length) {
      ...
      placements = setPlacementsForResource(placementMap, resourceId, [
        {
          placementKey: buildPlacementKey(resourceId, 0),
          trackName,
    ...
    if (kind === "audio") {
      for (const placement of placements) {
        const placementKey = String(placement?.placementKey || buildPlacementKey(resourceId, 0));
        const stereoTrackName = String(placement?.trackName || "").trim() || "Audio S1";
        const monoTrackName = deriveMonoTrackNameFromStereo(stereoTrackName, kindCounter.audio);
        ...
        const assetKey = `audio:${resourceId}:${placementKey}:stereo`;
...
export function renderLoopCompositionStudioView({
  ...
  if (!resources.length) {
    ...
    for (const resource of resources) {
      const resourceId = String(resource?.id || "");
      const placement = getPrimaryPlacementForResource(getPlacementMap(scopeKey), resourceId) || null;
  ...
  renderStudioTimeline({
    ...
    onDropResource: ({ resourceId, resourceKind, trackName, timeSec }) => {
      ...
      const previous = getPrimaryPlacementForResource(scopePlacements, id) || {};
      const durationSec = Math.max(0.1, toFiniteNumber(previous.durationSec, defaultClipDurationSec(resource)) || 0.1);
      setPlacementsForResource(scopePlacements, id, [{
        placementKey: buildPlacementKey(id, 0),
        trackName: track,
    ...
    onClipEdit: ({ clipId, resourceId, trackName, trackKind, timeSec, durationSec }) => {
      ...
      upsertPlacementForResource(scopePlacements, id, String(clipId || "").trim(), {
    ...
    onClipCut: ({ clipId, resourceId, trackKind, cutTimeSec }) => {
      ...
      if (!resource) return false;
      const resourceKind = String(resource?.kind || "").toLowerCase();
      if (resourceKind !== "video" || String(trackKind || "").toLowerCase() !== "video") return false;
      const scopePlacements = getPlacementMap(scopeKey);
