...
import { normalizeTrackRowScale } from "../../domain/services/track_layout.js";
import { resolveEffectiveSelectedClipCount } from "../../domain/services/selection_state.js";
  ...
  if (state.shortcutsLabel) {
    ...
    if (signature !== state.shortcutsSignature) {
      ...
      for (const item of hints) {
        const chip = el("span", { class: "lemouf-studio-shortcut-chip" });
        chip.append(
          createIcon(item.icon, {
            className: "lemouf-studio-shortcut-icon",
            size: 12,
            title: item.text,
          }),
          el("span", { class: "lemouf-studio-shortcut-text", text: item.text })
        );
