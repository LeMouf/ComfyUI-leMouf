export function createTimelineShell(config = {}) {
  ...
  const nav = el("div", { class: "lemouf-studio-toolbar-group" });
  const overviewLabel = el("div", {
    class: "lemouf-studio-toolbar-overview",
    text: "",
  });
  const statusLabel = el("div", {
    class: "lemouf-studio-toolbar-status",
    text: "Stopped · 0.00s",
  });
  ...
  const sectionVizSelect = el("select", { class: "lemouf-loop-select lemouf-studio-viz-select" });
  ...
  const canvasWrap = el("div", { class: "lemouf-studio-arrange-canvas-wrap" });
  const canvas = el("canvas", { class: "lemouf-studio-arrange-canvas" });
  canvasWrap.appendChild(canvas);
  layout.append(canvasWrap);
  const footer = el("div", { class: "lemouf-studio-footer" });
  const shortcutsGroup = el("div", {
    class: "lemouf-studio-footer-group lemouf-studio-footer-shortcuts-wrap",
  });
  const shortcutsLabel = el("span", {
    class: "lemouf-studio-footer-shortcuts",
    text: "",
  });
  shortcutsGroup.append(shortcutsLabel);
  const footerActions = el("div", { class: "lemouf-studio-footer-actions" });
  const zoomGroup = el("div", { class: "lemouf-studio-footer-group" });
  const zoomLabel = el("span", { class: "lemouf-studio-footer-zoom", text: "zoom n/a" });
  ...
  body.classList.toggle("lemouf-studio-body-compact", compactMode);
